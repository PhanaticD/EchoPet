...
public abstract class EntityPet extends EntityCreature implements IAnimal, IEntityPet {
	...
	*/
	...
	public void g(float sideMot, float forwMot){// Look at EntityHorse for shit
		// bv() is passenger shit. Minecraft changed it from 1 passenger to a list
		if(bv().isEmpty()){
			this.P = 0.5F;
			this.aQ = 0.02F;
		...
		Entity passenger = this.bv().get(0);
		...
		sideMot = ((EntityLiving) passenger).be * 0.5F;// 1 after lastDamage
		forwMot = ((EntityLiving) passenger).bf;// After ^
