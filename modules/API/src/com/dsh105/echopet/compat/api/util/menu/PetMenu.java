		...
		for(MenuOption o : this.options){
			if(o.item.getMenuType() == DataMenu.DataMenuType.BOOLEAN){
				...
				if(GeneralUtil.isEnumType(PetData.class, mi.toString())){
					...
					if(pd.getVersion().isCompatible(new Version())){
						if(pet.getPetData().contains(pd)){
							this.inv.setItem(o.position - incompatible, o.item.getBoolean(false));
						}else{
							this.inv.setItem(o.position - incompatible, o.item.getBoolean(true));
						}
					}else{
						incompatible++;
					}
				...
				this.inv.setItem(o.position - incompatible, o.item.getItem());
			}
		}
		int book = size - 1;
